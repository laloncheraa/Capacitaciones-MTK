(function () {
  ...
  function renderDashboard() {
    ...
    elements.downloadSummary.textContent = `La descarga respetará la región del usuario y los filtros activos. Registros listos: ${records.length}.`;
  ...
  function handleLogin(event) {
    ...
    if (normalize(user.status) !== "activo") {
      elements.loginMessage.textContent = "El usuario está inactivo.";
      return;
    }

    if (password !== config.mockPassword) {
      elements.loginMessage.textContent = "Contraseña demo incorrecta.";
      return;
    }
